#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""B0: steering Gaussian NLL + weighted SmoothL1(e_y); export/eval still use [steering,e_y]."""
...
class SeqCfCNLL(RegressionSequenceCfCSteeringNet):
    """reg_head outputs [steering, e_y, log_sigma_steer]."""
    ...
    def forward(self, input_tensor: torch.Tensor, return_aux: bool = False):
        ...
        out = self.reg_head(hidden)
        if return_aux:
            return out, self.aux_head(feat_seq[:, -1, :])
        return out
...
def nll_loss(pred, labels, ey_quality, ey_loss_weight: float):
    ...
    # Gaussian NLL: 0.5*( ((y-mu)/sig)^2 + 2*log_sig + log(2pi) )
...
def parse_args():
    p = argparse.ArgumentParser(description="B0 NLL challenger")
...
def main():
    ...
    model = SeqCfCNLL(num_frames=3).to(device)
    ...
    write_verdict(out / "verdict.md", experiment="B0_nll", hmm_mae=hmm["hmmSteeringMAE"], raw_mae=hmm["rawSteeringMAE"])
