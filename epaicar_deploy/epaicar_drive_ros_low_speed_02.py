...
class MotionController(object):
    def __init__(self):
        ...
        self.camera = str(rospy.get_param("~camera", "/dev/video1"))
        ...
        self.slow_linear_speed = float(rospy.get_param("~slow_linear_speed", 0.16))
        self.angular_scale = float(rospy.get_param("~angular_scale", 0.4))
    ...
    def _start_infer_server(self):
        ...
        cmd = [
            ...
            "--angular-scale", str(self.angular_scale),
            "--max-angular", str(self.max_angular_z),
            "--print-every", "60",
        ]
        rospy.loginfo("Starting temporal3 2D TensorRT UDP server with angular_scale=%.3f: %s", self.angular_scale, " ".join(cmd))
