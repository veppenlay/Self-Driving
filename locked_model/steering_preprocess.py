...
class PreprocessConfig:
    ...
    use_roi: bool = False
    illumination_profile: str = "none"


DEFAULT_PREPROCESS_CONFIG = PreprocessConfig()
DEFAULT_ROI_BOTTOM_RATIO = 0.7
ANGLE_KEY_DECIMALS = 6
ILLUMINATION_PROFILE_NONE = "none"
ILLUMINATION_PROFILE_HSV_V_PERCENTILE_5_95 = "hsv_v_percentile_5_95"
ILLUMINATION_PROFILE_HSV_V_PERCENTILE_30_98 = "hsv_v_percentile_30_98"
ILLUMINATION_PROFILE_HSV_V_PERCENTILE_56_99 = "hsv_v_percentile_56_99"
ILLUMINATION_PROFILE_PERCENTILE_RANGES = {
    ILLUMINATION_PROFILE_HSV_V_PERCENTILE_5_95: (5.0, 95.0),
    ILLUMINATION_PROFILE_HSV_V_PERCENTILE_30_98: (30.0, 98.0),
    ILLUMINATION_PROFILE_HSV_V_PERCENTILE_56_99: (56.0, 99.0),
}
SUPPORTED_ILLUMINATION_PROFILES = {
    ILLUMINATION_PROFILE_NONE,
    *ILLUMINATION_PROFILE_PERCENTILE_RANGES.keys(),
}
...
def _normalize_profile_name(profile: str | None) -> str:
    profile_name = (profile or ILLUMINATION_PROFILE_NONE).strip().lower()
    if profile_name not in SUPPORTED_ILLUMINATION_PROFILES:
        raise ValueError(f"unsupported illumination_profile: {profile}")
    return profile_name


def apply_illumination_profile_hwc(
    image_hwc: np.ndarray,
    *,
    color_space: str,
    illumination_profile: str,
) -> np.ndarray:
    profile = _normalize_profile_name(illumination_profile)
    if profile == ILLUMINATION_PROFILE_NONE:
        return image_hwc

    if color_space != "hsv":
        raise ValueError(f"illumination_profile={profile} requires color_space='hsv', got {color_space!r}")
    if image_hwc.ndim != 3 or image_hwc.shape[2] != 3:
        raise ValueError(f"expected HWC image with 3 channels, got shape={image_hwc.shape}")

    percentile_range = ILLUMINATION_PROFILE_PERCENTILE_RANGES.get(profile)
    if percentile_range is not None:
        low_percentile, high_percentile = percentile_range
        output = image_hwc.copy()
        v_channel = output[..., 2].astype(np.float32)
        v_low = float(np.percentile(v_channel, low_percentile))
        v_high = float(np.percentile(v_channel, high_percentile))
        if v_high - v_low > 1e-6:
            normalized = np.clip((v_channel - v_low) / (v_high - v_low), 0.0, 1.0) * 255.0
            output[..., 2] = np.clip(normalized, 0.0, 255.0).astype(np.uint8)
        return output
...
def preprocess_bgr_to_hwc_float(
    ...
    *,
    ...
    converted = apply_illumination_profile_hwc(
        converted,
        color_space=config.color_space,
        illumination_profile=config.illumination_profile,
    )
...
def preprocess_config_to_dict(config: PreprocessConfig) -> dict[str, object]:
    return {
        ...
        "roiBottomRatio": float(DEFAULT_ROI_BOTTOM_RATIO),
        "illuminationProfile": config.illumination_profile,
    }
...
def preprocess_config_from_dict(data: dict | None, *, fallback: PreprocessConfig = DEFAULT_PREPROCESS_CONFIG) -> PreprocessConfig:
    ...
    illumination_profile = _normalize_profile_name(
        str(data.get("illuminationProfile", data.get("illumination_profile", fallback.illumination_profile)))
    )
